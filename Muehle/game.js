(function () {
    ...
    function startButtonClick() { if (!gameStarted) { const continueMatch = matchSelect.value === "match" && matchRunning && (state?.winner || state?.draw) && matchWins[1] < 2 && matchWins[2] < 2; if (!continueMatch) { matchWins = { 1: 0, 2: 0 }; matchRound = 1; matchRunning = matchSelect.value === "match"; } startRound(); } else { abortGame(); } }
